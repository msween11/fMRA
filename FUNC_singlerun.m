function [recfft, recspace, recspace_opt, l8_freq_rel_err,...
    l8_hfix_space_err, l8_hopt_space_err, plotting, h_opt] = FUNC_singlerun...
    (seed, N,l,B,lambda,sigma,n,hfix,r, zero_cond,signal, shifts, thresh)
...
h_opt = hrange(find(h_errs == min(h_errs), 1, 'first'));
rec = zeros(1, length(Xext));
for j = 1:length(Xext)
    rec(j) = (wgap/(2*pi)).* sum(exp(1i.*Wext*Xext(j)).*...
